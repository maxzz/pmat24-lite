...
export function setZoomActionInMain(action: R2M.SetZoomAction['action']) {
    ...
    const newLevel = calculateNextZoomLevel(currentPercent, action);
...
function calculateNextZoomLevel(currentPercent: number, action: R2M.SetZoomAction['action']): number {
