...
import { confirmDeleteCpassMessages, confirmDeleteMessages, doAsyncExecuteConfirmDialogAtom } from "@/components/4-dialogs/5-confirm/1-confirmation";
...
import { inTest_Set } from "@/store/0-serve-atoms/6-do-in-use-test";
...
export const doDeleteFileUsAtom = atom(null,
    async (get, set, fileUsAtom: FileUsAtom) => {
        ...
        if (res) {
            notice.error(`Cannot delete file: ${res}`);
            return;
        }
