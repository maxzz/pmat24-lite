...
export const setHowChangedAtom = atom(
    null,
    (get, set, { options, how }: { options: FormOptionsState.AllAtoms, how: Matching.How; }) => {
        const getset = { get, set };
        const { ourlAtom, murlAtom } = options.p2Detect;
        const { murl_howAtom, murl_optAtom, murl_regexAtom } = options; // parts of string 'how:opt:url'
...
export const updateAfterRegexUrlChangeAtom = atom(
    ...
    (get, set, { oFormCtx }: { oFormCtx: FormOptionsState.AllAtoms; }) => {
        const getset = { get, set };
        const { ourlAtom, murlAtom } = oFormCtx.p2Detect;
        const { murl_howAtom, murl_optAtom, murl_regexAtom } = oFormCtx;
