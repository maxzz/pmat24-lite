 ...
 */
export function getAsFileSystemHandleFromEntry(
    ...
    return {
        kind: entry.isFile ? 'file' : entry.isDirectory ? 'directory' : (undefined as never),
        name: entry.name,
        ...
        async *values(): ReturnType<FileSystemDirectoryHandle['values']> {
            ...
            const entries = await new Promise<Array<NonNullable<ReturnType<typeof getAsFileSystemHandleFromEntry>>>>(
                (resolve) => {
...
export default async function* getFilesFromDataTransfer(
    ...
    const fileSystemHandles = await Promise.all(
        Array.from(dataTransfer.items, async (item) => {
            let fileSystemHandle: | FileSystemFileHandle | FileSystemDirectoryHandle | null | undefined;
            ...
            function getAsEntry(): ReturnType<DataTransferItem['webkitGetAsEntry']> {
                return typeof (item as any).getAsEntry === 'function' ? (item as any).getAsEntry() : item.webkitGetAsEntry();
            }
