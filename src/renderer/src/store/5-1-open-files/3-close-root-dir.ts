    ...
    async (get, set): Promise<boolean> => {
        ...
        try {
            typeof tmApi !== 'undefined'
                && tmApi.invokeMain({ type: 'r2mi:debug-log', payload: { sessionId: '327545', runId: 'open-folder-pre', hypothesisId: 'H_CLOSE', location: 'src/renderer/src/store/5-1-open-files/3-close-root-dir.ts:doCloseRootDirAtom:entry:ipc', message: 'doCloseRootDirAtom entry', data: { confirmExit, hasUnsavedChanges: !!allFileUsChanges.size, unsavedChangesCnt: allFileUsChanges.size }, timestamp: Date.now(), } }).catch(() => { });
        } catch { }
        // #endregion
        ...
        try {
            ...
            if (!okToClose) {
                ...
                try {
                    typeof tmApi !== 'undefined'
                        && tmApi.invokeMain({ type: 'r2mi:debug-log', payload: { sessionId: '327545', runId: 'open-folder-pre', hypothesisId: 'H_CLOSE', location: 'src/renderer/src/store/5-1-open-files/3-close-root-dir.ts:doCloseRootDirAtom:cancelled:ipc', message: 'doCloseRootDirAtom cancelled', data: { confirmExit, unsavedChangesCnt: allFileUsChanges.size }, timestamp: Date.now(), } }).catch(() => { });
                } catch { }
                // #endregion
                return false;
            }
            ...
            try {
                typeof tmApi !== 'undefined'
                    && tmApi.invokeMain({ type: 'r2mi:debug-log', payload: { sessionId: '327545', runId: 'open-folder-pre', hypothesisId: 'H_CLOSE', location: 'src/renderer/src/store/5-1-open-files/3-close-root-dir.ts:doCloseRootDirAtom:exception:ipc', message: 'doCloseRootDirAtom exception', data: error instanceof Error ? { message: error.message, stack: error.stack } : { errorType: typeof error, error: String(error) }, timestamp: Date.now(), } }).catch(() => { });
            } catch { }
            // #endregion
            throw error;
        }
