...
const open_NewManiDlgAtom = atom(() => null, (get, set, openManiDlgData: NewManiDlgData) => set(doOpenDlgNewManiSawAtom, openManiDlgData));
...
export async function asyncExecuteNewManiDlg(set: Setter): Promise<boolean> {
    const ok = await (new Promise<boolean>((resolve) => { set(open_NewManiDlgAtom, { resolve, }); }));
    return ok;
}
