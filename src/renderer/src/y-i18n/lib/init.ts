...
export type RegisteredTranslations =
    Register extends { translations: infer T; }
...
type ParseArgType<ParamType extends string, ParamName extends string, Enums extends EnumMap> =
...
type Params<S extends DotPathsFor> = ExtractParamArgs<
    ...
    NormalizedTranslationAtKeyWithParams<S>[1] extends { enum: infer E; }
...
export function initI18n({ locale, fallbackLocale, translations, }: { locale: string; fallbackLocale: string | string[]; translations: Record<Lowercase<string>, LanguageMessages>; }) {
...
function getTranslation<S extends DotPathsFor, A extends Params<S>>(locale: string, translations: LanguageMessages, key: S, args?: A) {
...
function performSubstitution(locale: string, str: string, args: Record<string, unknown>, translationParams: ParamOptions): string {
