//https://github.com/radix-ui/primitives/blob/74b182b401c8ca0fa5b66a5a9a47f507bb3d5adc/packages/react/select/src/Select.tsx "version": "2.1.2", 11.03.24
...
const [createSelectContext, createSelectScope] = createContextScope(SELECT_NAME, [createCollectionScope, createPopperScope,]);
...
const [SelectNativeOptionsProvider, useSelectNativeOptionsContext] = createSelectContext<SelectNativeOptionsContextValue>(SELECT_NAME);
            ...
            >
                <Collection.Provider scope={__scopeSelect}>
                    <SelectNativeOptionsProvider
                        ...
                        onNativeOptionAdd={React.useCallback((option) => { setNativeOptionsSet((prev) => new Set(prev).add(option)); }, [])}
                        onNativeOptionRemove={React.useCallback((option) => { setNativeOptionsSet((prev) => { const optionsSet = new Set(prev); optionsSet.delete(option); return optionsSet; }); }, [])}
                    >
                        {children}
                    </SelectNativeOptionsProvider>
                </Collection.Provider>
...
interface SelectValueProps extends Omit<PrimitiveSpanProps, 'placeholder'> { placeholder?: React.ReactNode; }
...
type SelectContentContextValue = {
    ...
    onItemLeave?: () => void;
    itemTextRefCallback?: (node: SelectItemTextElement | null, value: string, disabled: boolean) => void;
...
interface SelectContentImplProps
    ...
    onEscapeKeyDown?: DismissableLayerProps['onEscapeKeyDown']; // Event handler called when the escape key is down. Can be prevented.
    onPointerDownOutside?: DismissableLayerProps['onPointerDownOutside']; //Event handler called when the a `pointerdown` event happens outside of the `DismissableLayer`. Can be prevented.
...
const SelectContentImpl = React.forwardRef<SelectContentImplElement, SelectContentImplProps>(
    (props: ScopedProps<SelectContentImplProps>, forwardedRef) => {
        ...
        // prevent selecting items on `pointerup` in some cases after opening from `pointerdown` and close on `pointerup` outside.
        ...
        return (
            ...
            >
                <RemoveScroll as={Slot} allowPinchZoom>
                    <FocusScope
                        asChild
                        trapped={context.open} // we make sure we're not trapping once it's been closed (closed !== unmounted when animating out)
                        onMountAutoFocus={(event) => { event.preventDefault(); }} // we prevent open autofocus because we manually focus the selected item
                        onUnmountAutoFocus={composeEventHandlers(onCloseAutoFocus, (event) => { context.trigger?.focus({ preventScroll: true }); event.preventDefault(); })}
                    >
                        <DismissableLayer
                            asChild
                            ...
                            onFocusOutside={(event) => event.preventDefault()} // When focus is trapped, a focusout event may still happen. We make sure we don't trigger our `onDismiss` in such case.
                        ...
                        >
                            <SelectPosition
                                ...
                                ref={composedRefs}
                                style={{
                                    display: 'flex', // flex layout so we can place the scroll buttons properly
                                    flexDirection: 'column',
                                    outline: 'none', // reset the outline by default as the content MAY get focused
                                    ...contentProps.style,
                                }}
                                onKeyDown={composeEventHandlers(contentProps.onKeyDown, (event) => {
                                    const isModifierKey = event.ctrlKey || event.altKey || event.metaKey;

                                    if (event.key === 'Tab') event.preventDefault(); // select should not be navigated using tab key so we prevent it
            ...
            if (
                ...
                if (willAlignWithoutTopOverflow) {
                    ...
                    const clampedTriggerMiddleToBottomEdge = Math.max(
                        ...
                        (isLastItem ? viewportPaddingBottom : 0) + // viewport might have padding bottom, include it to avoid a scrollable viewport
                    ...
                    const clampedTopEdgeToTriggerMiddle = Math.max(
                        ...
                        viewport.offsetTop +
                        (isFirstItem ? viewportPaddingTop : 0) + // viewport might have padding top, include it to avoid a scrollable viewport
                        selectedItemHalfHeight
                    );
