...
function MenuItem({ label, theme, current }: { label: string; theme: ThemeMode; current: ThemeMode; }) {
    return (
        <DropdownMenuItem className="text-xs grid grid-cols-[16px,1fr] items-center gap-x-2" onClick={() => appSettings.appUIOptions.theme = theme}>
...
export function ThemeSwitch() {
    const { theme: snapTheme } = useSnapshot(appSettings).appUIOptions;
    ...
    return (
        <div className="focus-within:ring-1 focus-within:ring-ring rounded-md flex items-center">
            <Button
                ...
                onClick={() => appSettings.appUIOptions.theme = appSettings.appUIOptions.theme === 'dark' ? 'light' : 'dark'}
            >
