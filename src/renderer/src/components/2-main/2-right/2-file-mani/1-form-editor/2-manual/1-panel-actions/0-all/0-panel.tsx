...
export function ManualPanelActions({ mFormCtx, className, ...rest }: { mFormCtx: MFormContextProps; } & HTMLAttributes<HTMLDivElement>) {
    const listRef = useRef<HTMLDivElement>(null);

    const initCb = useInitSelectedIdx(mFormCtx.mAllAtoms.manual);
    useEffect(() => { initCb(); }, []);

    const doCreateItem = useSetAtom(doCreateItemAtom);

    async function onCreateNewManual(type: ChunkKey, password: boolean) {
        doCreateItem(mFormCtx, type, password);
    ...
    return (
        <div className={classNames("p-1 pr-0 h-full border-border border rounded flex flex-col space-y-1 select-none", focusWithinClasses, className)} {...rest}>
            ...
            <PanelActionsTitle ctx={mFormCtx} addNew={onCreateNewManual} />

            <div className="relative h-full">
                <div className="absolute inset-0">

                    <ScrollArea className="pr-1 h-full">
                        <PanelActionsList mFormCtx={mFormCtx} ref={listRef} />
                    </ScrollArea>
