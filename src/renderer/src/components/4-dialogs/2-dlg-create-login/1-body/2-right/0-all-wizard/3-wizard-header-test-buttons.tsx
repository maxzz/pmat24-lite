...
import { type TestAppEnum, type TestScreenEnum, debugSettings, testApp, testScreen } from "@/store/1-atoms/9-ui-state";
...
export function DebugButtons({ className, ...rest }: ComponentPropsWithoutRef<'div'>) {
    ...
    return (
        <div className={classNames("px-2 py-0.5 text-[.67rem] grid grid-cols-[auto_auto_auto_auto] grid-rows-2 gap-x-2", className)} {...rest}>
            ...
            screen:
            <RadioGroup className="grid-cols-subgrid col-span-3" value={screen} onValueChange={(v) => debugSettings.testCreate.screen = v as TestScreenEnum}>
