...
import { type GetTlwScreenshotsResult, type TlwScreenshot } from "@shared/ipc-types";
...
export function AppsGrid({ className, ...rest }: ComponentProps<"div">) {
    ...
    return (
        <div className={classNames("grid gap-4 grid-cols-[repeat(auto-fill,minmax(150px,1fr))] 1debug", className)} {...rest}>
            {data.map(
                (item, idx) => {
                    return <RenaderTlwScreenshot item={item} idx={idx} />;
                }
            )}
        </div>
    );
...
function RenaderTlwScreenshot({ item, idx }: { item: TlwScreenshot, idx: number; }) {
    if (item.type === 'error') {
        // const res = null;
        const res = (
